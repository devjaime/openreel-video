...
import {
  ...
  getStoredGoogleApiKey, saveGoogleApiKey, clearGoogleApiKey,
  getActiveAiProvider,
...
const KeyRow: React.FC<{
  ...
  active: boolean; // this provider is currently selected by getActiveAiProvider
}> = ({ label, logo, placeholder, docsUrl, docsLabel, storedKey, onSave, onClear, onTest, active }) => {
  ...
  return (
    <div
      className={`rounded-lg border px-3 py-2.5 space-y-2 transition-all ${
        active
    ...
    >
      ...
      {storedKey ? (
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            <code className="flex-1 font-mono text-[10px] bg-background-tertiary px-2 py-1 rounded text-text-muted truncate">
              {showKey ? storedKey : masked}
            </code>
            <button
          ...
          <div className="flex gap-1.5">
            <button
              onClick={handleTest}
              disabled={testing}
              className="flex-1 flex items-center justify-center gap-1 text-[10px] py-1 rounded border border-border/30 hover:border-border text-text-muted hover:text-text-secondary transition-colors"
            >
              {testing ? <Loader2 className="w-3 h-3 animate-spin" /> :
               testResult === "ok" ? <Check className="w-3 h-3 text-green-400" /> :
               testResult === "error" ? <AlertTriangle className="w-3 h-3 text-red-400" /> : null}
              {testing ? "Probando…" :
               testResult === "ok" ? "OK ✓" :
               testResult === "error" ? "Error" : "Probar"}
            </button>
            <button
              onClick={() => { onClear(); setTestResult(null); }}
              className="flex items-center gap-1 text-[10px] py-1 px-2 rounded border border-red-500/20 hover:border-red-500/40 text-red-400/60 hover:text-red-400 transition-colors"
            >
              <X className="w-3 h-3" />
            </button>
          </div>
        ...
        <div className="flex gap-1.5">
          <input
            ...
            className="flex-1 text-[10px] bg-background-tertiary border border-border/30 rounded px-2 py-1.5 text-text-primary placeholder:text-text-muted/40 focus:outline-none focus:border-primary/40"
          />
          <button
            onClick={handleSave}
            disabled={!inputVal.trim()}
            className="text-[10px] px-2.5 py-1.5 rounded bg-primary/20 hover:bg-primary/30 text-primary disabled:opacity-40 transition-colors"
          >
            Guardar
          </button>
        </div>
...
// ──────────────────────────────────────────────────────────────────────────────
// AI Keys section (Google + OpenRouter)
// ──────────────────────────────────────────────────────────────────────────────
...
const ApiKeySection: React.FC = () => {
  ...
  const orKey = getStoredApiKey();
  const hasAny = !!(googleKey || orKey);
    ...
    <div className="border border-border/30 rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-text-secondary">Claves de IA</span>
          {hasAny && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-400 border border-green-500/20">
              {activeProvider === "google" ? "Google AI" : "OpenRouter"}
            </span>
          )}
          {!hasAny && (
      ...
      {open && (
        <div className="px-3 pb-3 space-y-2 border-t border-border/20">
          <p className="text-[10px] text-text-muted mt-2 leading-relaxed">
            Necesarias para miniaturas con IA y mejora de reels.
            Se guardan solo en tu navegador — nunca se envían a ningún servidor.
            <br />
            <span className="text-green-400/80">Google AI tiene plan gratuito generoso.</span>
          </p>

          {/* Google AI (preferred) */}
          <KeyRow
            label="Google AI (Gemini + Imagen 3)"
            logo="🇬"
            placeholder="AIza…"
            ...
            onSave={(k) => { saveGoogleApiKey(k); refresh(); }}
            onClear={() => { clearGoogleApiKey(); refresh(); }}
            onTest={async () => {
              const { GoogleAiClient } = await import("@openreel/core");
              const key = getStoredGoogleApiKey();
              if (!key) return { ok: false, error: "No key" };
              return new GoogleAiClient(key).testConnection();
            }}
          />

          {/* OpenRouter (fallback) */}
          <KeyRow
            label="OpenRouter (DALL-E 3 / GPT-4o)"
            logo="🔀"
            ...
            onSave={(k) => { saveApiKey(k); refresh(); }}
            onClear={() => { clearApiKey(); refresh(); }}
            onTest={async () => {
              const { OpenRouterClient } = await import("@openreel/core");
              const key = getStoredApiKey();
              if (!key) return { ok: false, error: "No key" };
              return new OpenRouterClient({ apiKey: key }).testConnection();
            }}
          />

          <p className="text-[9px] text-text-muted/60 text-center">
            Si tienes ambas claves, se usa Google AI primero.
          </p>
        </div>
      )}
    </div>
